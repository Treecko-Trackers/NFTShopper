...
router.get('/category/animal/:animal', async (req, res, next) => {
  try {
    const nfts = await NFT.findAll()
    res.send(nfts.filter(nft => nft.dataValues.features[0] === req.params.animal))
    }
    catch (err) {
    next(err)
  }
...
router.get('/category/hat/:withHat', async (req, res, next) => {
  try {
    const nfts = await NFT.findAll()
    res.send(nfts.filter(nft => nft.dataValues.features[1] === req.params.withHat))
    }
    catch (err) {
    next(err)
  }
...
router.get('/category/background/:color', async (req, res, next) => {
  try {
    const nfts = await NFT.findAll()
    res.send(nfts.filter(nft => nft.dataValues.features[2] === req.params.color))
    }
    catch (err) {
    next(err)
  }
